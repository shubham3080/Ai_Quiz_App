...
export default function CategoryClient({ params }: PageProps) {
  ...
  const loadMoreSubCategories = async () => {
    ...
    try {
      ...
      setError(`Failed to load subcategories ${err}`);
    } finally {
      setIsLoadingMore(false);
    }
  };
          ...
          <div className="lg:w-3/4">
            <div className="w-full sm:w-64">
              <input
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent mb-8"
              />
            </div>

            {isLoading ? (
              <MagicSearchLoader text="Loading Relavent Sub Paths"/>
            ) : (
              <div className="space-y-6 max-w-4xl">
                {subcategories.map((subcategory, index) => (
                  <SubcategoryCard
            ...
            {searchQuery === "" ? (
              <div ref={sentinelRef} className="h-10">
                {isLoading ? <></> : <MagicSearchLoader text="Loading More Paths.."/>}
              </div>
            ) : (
              <></>
            )}
